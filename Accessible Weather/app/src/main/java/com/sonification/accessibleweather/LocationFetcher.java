...
public class LocationFetcher extends Service
{
    ...
    public int onStartCommand(Intent intent, int flags, int startId)
    {
        ...
        Log.e("Location", "Started for location updates with forceConfidentLocation " + forceConfidentLocation);
            ...
            final Runnable method1Runnable = new Runnable() {
                public void run() {
                    if (method1State == STATE_TRYING) {
                        ...
                        getFineLocationFix();
                        Log.e("Location", "Method 1 timed out");
                    }
                }
            };
                ...
                final Runnable method2Runnable = new Runnable() {
                    public void run() {
                        if (method2State == STATE_TRYING) {
                            ...
                            locationManager.removeUpdates(locationListener);
                            getFineLocationFix();
                            Log.e("Location", "Method 2 timed out");
                        }
                    }
                };
    ...
    private boolean methodDatabase() {
        ...
        if(cacheCursor.moveToFirst())
        {
            ...
            cacheCursor.close();
            dbCache.close();
            Log.e("Location", "Database method success: " + latitude + ", " + longitude);
            return true;
        }
        cacheCursor.close();
        dbCache.close();
        Log.e("Location", "Database method failed");
        return false;
    }
    ...
    private boolean methodWundergroundIP() {
        ...
        Log.e("Location", "Auto IP success: " + latitude + ", " + longitude);
        return true;
    }
    ...
    private boolean methodLastKnown() {
        ...
        if (location == null) {
            Log.e("Location", "Method last known failed");
            return false;
        }
        else {
            longitude = Double.toString(location.getLongitude());
            latitude = Double.toString(location.getLatitude());
            Log.e("Location", "Method last known success: " + longitude + ", " + latitude);
            return true;
        }
    }
    ...
    private void sendMessage(int certainty) {
        ...
        Log.d("Location", "Sending Message..");
    ...
    LocationListener locationListener = new LocationListener()
    {
        ...
        public void onLocationChanged(Location location)
        {
            if(location != null)
            {
                latitude = Double.toString(location.getLatitude());
                longitude = Double.toString(location.getLongitude());
                Log.e("Location", "Fix received: " + latitude + ", " + longitude);
                if (method1State == STATE_TRYING) {
                    method1State = STATE_SUCCESS;
                    Log.e("Location", "Fix received using method 1");
                }
                if (method2State == STATE_TRYING) {
                    method2State = STATE_SUCCESS;
                    Log.e("Location", "Fix received using method 2");
                }
                sendMessage(CONFIDENT);
            }
            else
            {
                if (method1State == STATE_TRYING) {
                    method1State = STATE_FAILED;
                    Log.e("Location", "Method 1 returned null");
                }
                if (method2State == STATE_TRYING) {
                    method2State = STATE_FAILED;
                    Log.e("Location", "Method 2 returned null");
                }
                getFineLocationFix();
            }
        }
