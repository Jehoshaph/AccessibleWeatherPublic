...
public class WeatherFetcher
{
	...
	private boolean parseDailyString(String response)
	{
		...
		{
			dbDailyWeather.addRecord(dDay[j], dMonth[j], dYear[j], dTitle[j], dPop[j], dSnow[j], dHigh[j], dLow[j], dConditions[j]);
		}
    ...
    public boolean parseHourlyString(String response)
    {
        ...
        try
        {
            ...
            dMonth = new String[arrLength];
            dYear = new String[arrLength];
            dTemp = new String[arrLength];
            dConditions = new String[arrLength];
            dWindSpeed = new String[arrLength];
            dFeelsLikeTemp = new String[arrLength];
            dPop = new String[arrLength];
            ...
            for(int i = 0; i < arrLength; i++)
            {
                ...
                if(metric)
                {
                    dTemp[i] = hourlyObjects.getJSONObject(GlobalVariables.TEMP_KEY).getString(GlobalVariables.METRIC_KEY);
                    dWindSpeed[i] = hourlyObjects.getJSONObject(GlobalVariables.WIND_SPEED_KEY).getString(GlobalVariables.METRIC_KEY);
                    dFeelsLikeTemp[i] = hourlyObjects.getJSONObject(GlobalVariables.FEELS_LIKE_TEMP_KEY).getString(GlobalVariables.METRIC_KEY);
                }
                else
                {
                    dTemp[i] = hourlyObjects.getJSONObject(GlobalVariables.TEMP_KEY).getString(GlobalVariables.ENGLISH_KEY);
                    dWindSpeed[i] = hourlyObjects.getJSONObject(GlobalVariables.WIND_SPEED_KEY).getString(GlobalVariables.ENGLISH_KEY);
                    dFeelsLikeTemp[i] = hourlyObjects.getJSONObject(GlobalVariables.FEELS_LIKE_TEMP_KEY).getString(GlobalVariables.ENGLISH_KEY);
                }
            }
        }
        catch(JSONException e)
        ...
        for(int j = 0; j < arrLength; j++)
        {
            dbHourlyWeather.addRecord(dHour[j], dDay[j], dMonth[j], dYear[j], dTemp[j], dConditions[j], dWindSpeed[j], dFeelsLikeTemp[j], dPop[j]);
        }
